...
root_path = "/data/TheStack_IR/OJ_Samples/Collated/Rust"    
out_path = "/data/TheStack_IR/OJ_Samples/Scripts/Rust"
...
def compile_worker(dir_list_chunk):
    ...
    with (
        open(
            f"{out_path}/out_{current_process().pid}.txt", 
            "a+"
        ) 
        if os.path.exists(f"{out_path}/out_{current_process().pid}.txt") 
        else open(
            f"{out_path}/out_{current_process().pid}.txt", 
            "x+"
        ) as outfile,
        open(
            f"{out_path}/err_{current_process().pid}.txt", 
            "a+"
        ) 
        if os.path.exists(f"{out_path}/err_{current_process().pid}.txt")
        else open(
            f"{out_path}/err_{current_process().pid}.txt", 
            "x+"
        ) as errfile
        ...
        for folder in tqdm(
            ...
            src_path = f"{root_path}/{folder}/source.rs"
...
def main(args):
    ...
    with open(f"{out_path}/dir_list.pickle", "wb") as sp:
